...
- **Peer Review Note (2/15/2021)**: As I thought more about my submission (and noticed that it seemed longer to get reviews than Week 4), I decided that there were some things that I could improve in my report (so, I suspended my submision by starting to edit it).  Essentially, I did not include any statistical analysis, so I wanted to go back and either add or reference ANOVA or linear regression p-values (since I saw another peer's report that included a t-test p-value).  There was also a point about the STAR unique aligned reads versus the STAR unique alignment rate that was not precise and I think it would be better to correct during revision than as a post-acceptance correction (if I did get enough credit for the assignment, even with the previous submission).
