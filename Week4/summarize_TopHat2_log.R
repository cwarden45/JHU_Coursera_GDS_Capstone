alignment_folder = "hg19_TopHat2_Alignment"
GitHub_folder = "TopHat2_Alignment_Log_Files"
...
###samtools processing
Aligned_Pair_Count = c()
Aligned_Percent = c()

for (i in 1:nrow(metadata.table)){
	sampleID = metadata.table$SRR[i]
	log_file = paste(alignment_folder,"/",sampleID,"/align_summary.txt",sep="")
	log.content = readLines(log_file)

	log_copy = paste(GitHub_folder,"/",sampleID,"-align_summary.txt",sep="")
	command = paste("cp ",log_file," ",log_copy,sep="")
	system(command)
	
	reg.obj = gregexpr("(\\d+)",log.content[11])
	Aligned_Pair_Count[i] = as.numeric(unlist(regmatches(log.content[11], reg.obj)))

	reg.obj = gregexpr("(\\d+\\.\\d+)",log.content[9])
	Aligned_Percent[i] = as.numeric(unlist(regmatches(log.content[9], reg.obj)))
}#end for (i in 1:nrow(metadata.table))

output.table = data.frame(Sample=metadata.table$SRR, Group=metadata.table$Group,
							Total=total_pairs,
							Aligned_Pair_Count, Aligned_Percent)
write.table(output.table,"alignment_stats-TopHat2.txt", quote=F, sep="\t", row.names=F)